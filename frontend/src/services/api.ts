...
const apiClient = axios.create({
  ...
  timeout: 10000, // Adding timeout to detect connection issues
});

export const OperadorasService = {
  getOperadoras: async (filters: OperadorasFilter): Promise<OperadorasResponse> => {
    ...
    try {
      console.log('API request URL:', apiClient.defaults.baseURL + '/operadoras');
      ...
      return {
        items: response.data.data || [], // Map 'data' from backend to 'items' in our interface
